...
fulltext = input('Enter your text: ')
fulltext += "$"
y = input('Enter the sequence you searching for: ')
...
def create_sa(text):
    ...
    if len(set(s12_ranks)) == len(s12_ranks):
        s12 = [value[1]
               for value in sorted(s12_with_ranks, key=lambda x: x[0])]
    else:
...
def bucketsort(text, sa, s0=False):
    ...
    s12_with_ranks = [(ranks.get(''.join([str(x) for x in get_triplet(
        text, idx_original)])), idx_original) for idx_original in sa]
...
if __name__ == "__main__":
 ...
 # search the phrase
sa_id = search_suffix_binary(fulltext, sa, y, lcp_lr)
amount = count_results(sa_id, len(y), lcp)
...
if amount == 0:
    ...
    print("Your word:", y)
    print('Number of search term: ', amount)
    
suffixes = sorted([fulltext[i:] for i in range(len(fulltext))])

print("suffixes : ")
print(suffixes)
print("skew algorithme : ")
print(sa)
print("lcp : ")
print(lcp)
print("lcp_lr : ")
print(lcp_lr)
print("inverse skew algorithme : ")
print(inverse_sa)
